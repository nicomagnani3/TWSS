...
def addIndividual(node_type, label, url=BASE_URL):
...
def addRanting(movie, rating):
    rating_individual = BASE_URL["aggregateRating" +
                                 str(posicion("AggregateRating", url=BASE_SCHEMAORG_URL))]
    g.add((rating_individual, RDF.type, BASE_SCHEMAORG_URL["AggregateRating"]))
    if (rating is not None):
        g.add((rating_individual, BASE_SCHEMAORG_URL["ratingValue"], Literal(
            rating.get("ratingValue"), datatype=XSD.double)))
    g.add((movie, BASE_SCHEMAORG_URL["aggregateRating"], rating_individual))


def addImageSemanticWeb(imageurl):
    img_obj = BASE_URL["image" +
                       str(posicion("ImageObject", url=BASE_SCHEMAORG_URL))]
    img_obj
...
def addMovies(movie):
    ...
    pelicula
    g.add((URIRef(pelicula), BASE_SCHEMAORG_URL["duration"], Literal(
        movie.get("duration"), datatype=XSD.duration)))
    g.add((URIRef(pelicula), BASE_SCHEMAORG_URL["image"], addImageSemanticWeb(
        movie.get("image"))))
    if (movie.get("datePublished") is not None):
        g.add((URIRef(pelicula), BASE_SCHEMAORG_URL["datePublished"], Literal(datetime.strptime(
            movie.get("datePublished"), '%Y-%m-%d').isoformat(), datatype=XSD.date)))
    actor = []
    genre = []
    director = []
...
def addFunciones(movie):
    ...
    for funcion in movie.get("events"):
        urlSemanticWeb = BASE_URL["screeningEvent" +
                                  str(posicion("ScreeningEvent", url=BASE_SCHEMAORG_URL))]
        urlSemanticWeb
        funcion
        g.add((urlSemanticWeb, RDF.type, BASE_SCHEMAORG_URL["ScreeningEvent"]))
        g.add((urlSemanticWeb, BASE_SCHEMAORG_URL["videoFormat"], Literal(
            funcion.get("videoFormat"))))
        g.add((urlSemanticWeb, BASE_SCHEMAORG_URL["doorTime"], Literal(
            funcion.get("doorTime"))))
        ...
        g.add(
            (urlSemanticWeb, BASE_SCHEMAORG_URL["location"], cine_individual))
